...
def get_lvis(cam, scene, surf, normal):
    ...
    for i in range(n_lights):
        ...
        surf_frontlit = surf_frontlit + 0.01 * surf2l_frontlit
...
def func(in_dir, out_root):
    test_img = os.path.join(in_dir, "inputs/image_0000.png")
    h, w, _ = cv2.imread(test_img).shape

    imh = 512
    factor = h / imh
    print("Factor:", factor)

    h, w = round(h / factor), round(w / factor)
    print("Resized to:", h, w)

    train_cams = sorted(glob(os.path.join(in_dir, 'inputs/camera_*.txt')))
    val_cams = sorted(glob(os.path.join(in_dir, 'gt_camera_*.txt')))
    print(f"Found {len(train_cams)} train cams and {len(val_cams)} val cams")

    cams = [read_cam(cam, factor) for cam in train_cams + val_cams]  # K, R, t, T

    mesh_path = os.path.join(in_dir, 'neus_mesh.obj')
    mesh = o3d.t.io.read_triangle_mesh(mesh_path)

    scene = o3d.t.geometry.RaycastingScene()
    scene.add_triangles(mesh.vertex.positions.numpy(), mesh.triangle.indices.numpy().astype(np.uint32))

    counter = [0, 0]  # train, val
    for i, cam in tqdm(enumerate(cams)):
        if i >= len(train_cams):
            outdir = os.path.join(out_root, 'val_' + str(counter[1]).zfill(3))
            counter[1] += 1
        else:
            outdir = os.path.join(out_root, 'train_' + str(counter[0]).zfill(3))
            counter[0] += 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Extract nerfactor data from neus geometry')

    parser.add_argument('--input', required=True, type=str, help='input dataset folder')
    parser.add_argument('--out', required=True, type=str, help='save_folder')

    args = parser.parse_args()
    makedirs(args.out)

    func(args.input, args.out)
